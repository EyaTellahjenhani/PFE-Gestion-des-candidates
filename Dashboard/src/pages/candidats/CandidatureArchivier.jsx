...
const CandidatureArchivier = () => {
  ...
  const { data, error, isLoading,refresh,searchParams } = useFetch(() =>
    axios.get(API_BASE_URL + "/admin/candidate/archive?" + searchParams, { withCredentials: true })
  );
  ...
  return (
    <NavbarSidebarLayout>
      <div>
        <Breadcrumbe pageName1="Candidatures" pageName2="Les candidatures archivier" />

        <div className="flex flex-col sm:flex-row justify-between mt-4">
          <h2 className="text-lg font-bold sm:text-2xl">Les candidatures archivier</h2>
          <div className="flex gap-4 justify-center items-center">
          <Filter archive searchParams={searchParams} refresh={refresh} />
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl mt-4 p-4">
            <div className="flex flex-col gap-2">
              <div className="flex justify-between">
                <h3 className="text-lg font-bold">Liste des candidatures archivier</h3>
              </div>
              <hr className="my-2" />
                ...
                <table className="w-full text-sm text-left rtl:text-right border-collapse">
                  <thead className="text-xs text-[#A0AEC0]">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 whitespace-nowrap">
                        <div className="flex items-center">Date de traitement</div>
                      </th>
                  ...
                  <tbody>
                    {data.slice(pagesVisited, pagesVisited + RequestPerPage).map((c, i) => (
                      <tr className=" bg-white font-medium border-b" key={i}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {format(new Date(c.traitedAt), "dd/MM/yyyy HH:mm")}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          {c.status == "Accepté" ?
                            <span className="inline-flex items-center rounded-md bg-green-600 px-2 py-1 text-xs font-bold text-white ">
                              Accepté
                            </span> :
                            <span className="inline-flex items-center rounded-md bg-primary px-2 py-1 text-xs font-bold text-white ">
                              Refusé
                            </span>
                          }
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <div className="flex gap-2 text-[#718096]">
                            <Link to={"/dashboard/candidat/details/" + c._id}>
...
export default CandidatureArchivier;
